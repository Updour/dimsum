...
_onRetireveNumberPhoneContact = async () => {
      ...
      Contacts.getAll((err, contacts) => {
        if (err === 'denied'){
          ...
          this.setState({ 
            ...
            modalContact: true, 
            isShowNominal: true 
          }, ()=> this._onSetItemContactSortingA_Z())
            setTimeout(() => {
              this._onRetrieveValueDataPrefix()
            }, 1200);
  ...
  // 
  _onSetItemContactSortingA_Z = () => {
    let { contacts } = this.state;
    let results = contacts.sort((a, b) => a.name.localeCompare(b.name))
  ...
  render() {
    let { 
      ...
      isBuying, isChecking, isClicking, selectTypes, isShowPhone, modalContact, contact
    } = this.state;
    let { 
