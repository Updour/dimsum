    ...
    try {
      ...
      if (val) {
        ...
        if (parsed.upline && parsed.dealer === '1')
          ...
          this.setState({ userSd: results, isChangeMenu: false, isLoading: true })
        // sd
        if (parsed.upline === '1' && parsed.dealer !== '1') {
          let result = this.state.activity.filter(x => x.code === "xm" || x.code === "sd")
          this.setState({ userSd: result, isChangeMenu: false, isLoading: true })
        }
        // xm
      if (parsed.upline !== '1' && parsed.dealer !== '1') {
        let data = this.state.activity.filter(i => i.code === "xm")
        this.setState({ userXm: data, isChangeMenu: true, isLoading: true })
      }
