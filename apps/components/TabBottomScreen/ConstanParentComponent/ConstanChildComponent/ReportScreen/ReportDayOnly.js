...
export default class ReportDayOnly extends Component {
	_isMounted = false;
	state = {
		dpurchase: [],
		isModal: false,
	...
	_onRetrieveValueDataTransaction = async () => {
		try {
			...
			AsyncStorage.setItem('@isDay', JSON.stringify(data))
			if (this._isMounted) {
	...
	_onRetrieveDataFilteringDateNow = () => {
		...
		if (this._isMounted) {
			this.setState({ dayNow: data.reverse() })
		}
	}
	//
	...
	_onRetrieveValueNewData = () => {
		this.setState({ refreshing: true, dayNow: ''}, ()=> this._onRetrieveValueDataTransaction())
	}
	render() {
		let { 
			dayNow, agenid, area, harga, ket_sts, number_reorder, provider, tanggal, 
