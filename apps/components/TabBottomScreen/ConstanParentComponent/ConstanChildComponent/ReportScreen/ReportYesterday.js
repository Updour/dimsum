...
export default class ReportYesterday extends Component {
	...
	_onRetrieveDataFilteringYesterday = () => {
		...
		if (this._isMounted) {
			this.setState({ yesterDay: data.reverse() })
		}
	}
// 
	...
	_onRetrieveValueNewData = () => {
		this.setState({ refreshing: true, yesterDay: ''}, ()=> this._onRetrieveValueDataTransaction())
	}
	render() {
		let { 
			yesterDay, agenid, area, harga, ket_sts, number_reorder, provider, tanggal, 
