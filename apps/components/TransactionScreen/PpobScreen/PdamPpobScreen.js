...
import { 
  pNetpDam, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed, PickerDroid, Coders
} from '../../CollectionScreen'

import PdamScreenHeader from './PropsHeaderComponent/PdamScreenHeader'

export default class PdamPpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    selectCodes: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(pNetpDam())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <PdamScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
