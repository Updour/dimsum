...
import { 
  pNetTvs, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed, PickerDroid, Coders
} from '../../CollectionScreen'

import TvScreenHeader from './PropsHeaderComponent/TvScreenHeader'

export default class TvPpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    selectCodes: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(pNetTvs())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <TvScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
          <Card style={cardStyles}>
            <Form style={formStyles}>
            <Label style={selectCodes ? textItemA:textItemIn} >Code Product</Label>
              <Item>
