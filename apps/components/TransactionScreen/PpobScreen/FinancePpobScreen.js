...
import { 
  pNetFinances, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed, PickerDroid, Coders
} from '../../CollectionScreen'

import FinanceScreenHeader from './PropsHeaderComponent/FinanceScreenHeader'

export default class FinancePpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    selectCodes: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(pNetFinances())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <FinanceScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
