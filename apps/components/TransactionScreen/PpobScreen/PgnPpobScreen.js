...
import { 
	pNetPgN, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
	Empty, styles, Reply, timers, PleaseWait, Processed
} from '../../CollectionScreen'

import PgnScreenHeader from './PropsHeaderComponent/PgnScreenHeader'

export default class PgnPpobScreen extends Component {
	_isMounted = false;
	state = {
		codes: [],
		input: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(pNetPgN())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <PgnScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
	      <Content>
