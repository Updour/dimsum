...
import { 
  dpNetTselPhone, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed, PickerDroid, Coders
} from '../../CollectionScreen'

import ZonaPhoneTselScreenHeader from './PropsHeaderComponent/ZonaPhoneTselScreenHeader'

export default class ZonaTselPhonePpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    selectCodes: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(dpNetTselPhone())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <ZonaPhoneTselScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
