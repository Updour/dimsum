...
import { 
  dpNetTselDatas, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed, PickerDroid, Coders
} from '../../CollectionScreen'

import ZonaDataTselScreenHeader from './PropsHeaderComponent/ZonaDataTselScreenHeader'

export default class ZonaTselDataPpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    selectCodes: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(dpNetTselDatas())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <ZonaDataTselScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
