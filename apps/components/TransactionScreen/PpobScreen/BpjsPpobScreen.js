...
import { 
  pNetbPjs, netInbox, netOutbox, netInterval, reloaded, ReloadScreen, tags, pays, types, 
  Empty, styles, Reply, timers, PleaseWait, Processed
} from '../../CollectionScreen'

import BpjsScreenHeader from './PropsHeaderComponent/BpjsScreenHeader'

export default class BpjsPpobScreen extends Component {
  _isMounted = false;
  state = {
    codes: [],
    input: '',
  ...
  _onRetrieveValueProductNames = async () => {
    try {
      let results = await axios.get(pNetbPjs())
  ...
  render() {
    ...
    return (
      ...
      <ReloadScreen 
       ...
       >
      <BpjsScreenHeader {...this.props} onPress={this._onReloadScreenAndData}/>
        <Content>
