...
import { 
  vNetGopay, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import GopayHeader from './PropsHeaderComponent/GopayHeader'


export default class GopayVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetGopay())
 ...
 // switch to save data
  ...
  render() {
    ...
    return (
      <Container>
      <GopayHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
