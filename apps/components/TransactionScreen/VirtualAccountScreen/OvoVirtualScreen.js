...
import { 
  vNetOvo, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import OvoHeader from './PropsHeaderComponent/OvoHeader'


export default class OvoVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetOvo())
  ...
  render() {
    ...
    return (
      <Container>
      <OvoHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
            ...
            <Form style={formStyles}>
              <Item floatingLabel>
                <Icon name="ios-person" style={input ? iconAStyles : iconInStyles}/>
                  <Label style={input ? labelAStyles : labelInStyles}>Phone Number</Label>
                  <Input 
