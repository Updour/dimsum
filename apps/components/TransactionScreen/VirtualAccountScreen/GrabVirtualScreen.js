...
import { 
  vNetGrab, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import GrabHeader from './PropsHeaderComponent/GrabHeader'


export default class GrabVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetGrab())
  ...
  render() {
    ...
    return (
      <Container>
      <GrabHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
