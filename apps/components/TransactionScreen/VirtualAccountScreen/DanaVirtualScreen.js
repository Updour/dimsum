...
import { 
  vNetDana, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import DanaHeader from './PropsHeaderComponent/DanaHeader'


export default class DanaVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetDana())
  ...
  render() {
    ...
    return (
      <Container>
      <DanaHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
