...
import { 
  vNetEtoll, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import EtolHeader from './PropsHeaderComponent/EtolHeader'


export default class EtollVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetEtoll())
 ...
 // switch to save data
  ...
  render() {
    ...
    return (
      <Container>
      <EtolHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
