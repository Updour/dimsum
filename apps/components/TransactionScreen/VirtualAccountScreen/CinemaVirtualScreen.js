...
import CinemaHeader from './PropsHeaderComponent/CinemaHeader'


export default class CinemaVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetCinema())
  ...
  render() {
    ...
    return (
      <Container>
      <CinemaHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
