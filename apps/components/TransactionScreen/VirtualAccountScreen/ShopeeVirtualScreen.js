...
import { 
  vNetShopee, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import ShopeeHeader from './PropsHeaderComponent/ShopeeHeader'


export default class ShopeeVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try { 
      ...
      let results = await axios.get(vNetShopee())
  ...
  render() {
    ...
    return (
      <Container>
      <ShopeeHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
