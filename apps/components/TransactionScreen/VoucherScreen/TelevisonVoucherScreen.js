...
import { 
	vNetTv, styles, netInbox, timer, types, ModalPopUp,
...
import TvHeader from './PropsHeaderComponent/TvHeader'


export default class TelevisonVoucherScreen extends Component {
	_isMounted = false;
	state= {
		ArrCode: [],
		input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
  	try { 
  		...
  		let results = await axios.get(vNetTv())
  ...
  render() {
    ...
    return (
      <Container>
      <TvHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen 
