...
import { 
	netTransaction, timer, Empty, styles, SelectDate, SearchSubmit, UIActivityIndicator,
	ReloadScreen
} from '../../CollectionScreen'

import DataTransactionHeader from './PropsHeaderScreen/DataTransactionHeader'
import TransactionScreenResponse from './PropsResponseScreen/TransactionScreenResponse'

export default class DataTransactionScreen extends Component {
	_isMounted = false;
	state = {
		dataArr: [],
		refreshing: false,
  ...
  _onRetrieveValueDataResponse = async () => {
  	try {
  		let results = await axios.get(netTransaction() + this.state.id)
  		let data = results.data.data
  		AsyncStorage.setItem('@transaction', JSON.stringify(data));
  		if (this._isMounted) {
  ...
  _onRetrieveValueData = async () => {
  	let val = await AsyncStorage.getItem('@transaction')
  	let parsed = JSON.parse(val)
...
render() {
	...
	return (
		...
		>
			<DataTransactionHeader {...this.props} onPress={this._onReloadScreenAndData}/>
			<Content>
				...
				<FlatList 
					data={dataArr}
					renderItem={TransactionScreenResponse}
