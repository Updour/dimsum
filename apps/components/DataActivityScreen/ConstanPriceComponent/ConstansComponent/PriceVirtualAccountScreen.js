...
import { 
  netUsers, netVirtual, netTypeVirtual, netAllResult, PickerDroid, Submit, DotIndicator, tabStyles, 
  styles, ReloadScreen,  timer
} from '../../../CollectionScreen'

import VirtualAccountHeader from '../PropsHeader/VirtualAccountHeader'
...
export default class PriceVirtualAccountScreen extends Component {
  ...
  _onRetrieveValueDataProviderAndType = async () => {
    try {
      ...
      let promiseArray = [netVirtual(), netTypeVirtual() ].map(fetchURL);
  ...
  render() {
    ...
    return (
      <Container>
      <VirtualAccountHeader {...this.props} onPress={this._onRefreshingData} />
        <ReloadScreen 
