...
import { 
  ...
  styles, ReloadScreen, netType, timer
...
export default class PricePacketScreen extends Component {
  _isMounted = false
  state = {
    arrData: [],
    provider: [],
  ...
  _onRetrieveValueDataProviderAndType = async () => {
    try {
      ...
      let promiseArray = [netProvider(), netType() ].map(fetchURL);
  ...
  render() {
    let { arrData, refreshing, provider, typeSelected, selected, type } = this.state;
