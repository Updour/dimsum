...
import {
  ...
  Text, List, ListItem, Thumbnail, Footer, Input
} from 'native-base';
import { everyStyles, Submit, URL, formatPrice } from '../Container'


export default class ChangeCashier extends Component {
  state = {
    isCount: 0,
    isSetPrice: 0,
    isIcon: false,
  ...
  _onReceiveDetailProduct = async () => {
    try {
      ...
      const out = navigation.getParam('outlet')

      let uri = `${URL}detailcart/${out}/${val}`
