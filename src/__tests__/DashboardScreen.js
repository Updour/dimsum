...
class DashboardScreen extends Component {
    ...
    _onMainIncrement = async (val) => {
        if (_.isEqual(stock_product, 0)) return;
          this.setState({
            ...
            isSumPrice: (harga_jual_product) * (this.state.isCount + 1),
            isSnackbar: true
          })
    }
    ...
    _onMainDecrement = () => {
        ...
        this.setState({
          isCount: this.state.isCount - 1,
          isSumPrice: (this.state.isPrices) * (this.state.isCount - 1)
      })
    }
          ...
          <View key={i}>
            ...
            <TouchableOpacity onPress={() => this._onReceiveDetailProduct(id_product)}>
              ...
              <Right>
                ...
                {_.isEqual(stock_product, 0) ?  <Text note>Stock Kosong</Text> :
