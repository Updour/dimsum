  ...
  _onSaveValueCart = async () => {
    try {
      ...
      return _.map(isValProduct, item => {
        let counts = {
          id: item.id_product,
          img: item.gambar_product,
          info: item.keterangan_product,
          name: item.nama_product,
          outlet: item.outletid,
          slPrice: item.harga_jual_product,
          count: isCount,
          price: isSetPrice
        }
        AsyncStorage.getItem('cartItem').then(val => {
          const c = val ? JSON.parse(val) : [];
          c.push(counts)
          AsyncStorage.setItem('cartItem', JSON.stringify(c))
